...
io.on("connection", function (socket) {
  ...
  socket.on('game', function (message) {
    ...
    const aiMove: BestMove = findBestMove(serverGrid);
...
function AImove(grid : string[][], player: boolean) : number {
  // minmax
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[0].length; j++) {
        ...
        if (grid[i][j] === '-') {
          ...
          best = Math.max(best, AImove(grid, !player));         
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[0].length; j++) {
        ...
        if (grid[i][j] === '-') {
          ...
          // make the recursive call
          best = Math.min(best, AImove(grid, !player));
...
function findBestMove(board : string[][]) : BestMove {
  let bestRow = -1;
  let bestCol = -1;
  let bestVal = -100000;
  ...
  for (let i = 0; i < board.length; i++) {
    for (let j = 0; j < board.length; j++) {
      ...
      if (board[i][j] === '-') {
        ...
        let moveVal = AImove(board,false); // false because AI already made it's move. Now calculation will start from the user's move.
